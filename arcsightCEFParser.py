...
__name__ = 'Application to parse Arcsight CEF format log and insert into a Existing Hive table'
...
adsample = sc.textFile("hdfs://masternode/user/hdfs/adsample/Adlog1810.txt")
...
#get Date value to get yeaterday's year value
getdate = datetime.datetime.now() - datetime.timedelta(days = 1)
...
hive_context.sql("insert into table ambari_test.activedirlog select * from tab_name")

#write_list        
#d = [dict(x.split("=") for x in l) for l in main_list]
#for l in main_list:
#    for x in l:
        #print(re.split('(\w+)=',x).pop())
        #print(re.split('(\w+)=',x))
...
#"&&".join(str(r) for v in main_list for r in v)
