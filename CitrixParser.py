...
#__Version__ = '1.0'
#to run this file use below command in unix terminal
#to parse the processing day-1 ->$davar=$(date -d "-2 days" +"%Y%m%d") 
#to run the spark application ->$spark-submit /home/useraccount/pyspark_script/CitrixParser.py hdfs://namenode:54310/user/hdfs/Citrix/compressed/$davar/*.snappy $davar
#
...
#hive_context.setConf("hive.metastore.uris", "thrift://namenode:9083")
#SparkContext.setSystemProperty("hive.metastore.uris", "thrift://namenode:9083")
#spark = (SparkSession.builder.appName('CitrixlogParser').enableHiveSupport().getOrCreate())
spark = (SparkSession.builder.appName("CitrixLogParser").config("spark.executor.cores","2").config("spark.driver.memory", "6g").config("spark.executor.memory", "6g").config("hive.metastore.uris", "thrift://namenode:9083").config("spark.sql.hive.metastore.version", "2.1.1").config("spark.sql.hive.metastore.jars", "/usr/local/hive/lib/*").config("spark.sql.hive.metastore.sharedPrefixes", "com.mysql.jdbc").config("spark.sql.warehouse.dir", "hdfs://namenode:54310/user/hive/warehouse").config("hive.exec.dynamic.partition", "true").config("hive.exec.dynamic.partition.mode", "nonstrict").config("hive.exec.compress.output", "true").config("mapred.output.compression.codec", "org.apache.hadoop.io.compress.SnappyCodec").config("mapred.output.compression.type", "BLOCK").enableHiveSupport().getOrCreate())
